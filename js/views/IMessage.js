...
export default class extends Component{
  ...
  _changeColor = (index)=> {
    ...
    UIManager.measure(handle, (x, y, width, height, pageX, pageY) => {
      ...
      // view.setNativeProps({ style: {opacity: initOpacity} });
      this.setState({
        opacity:initOpacity
      })
    })
  ...
  render() {
    ...
    const linears = msg.map((elem, index) => {
      return (
          <View key={"linear"+index} ref={"msg"+index} colors={color[index]} style={[styles.linearGradient,{top:Util.size.height-30-(total-index)*50,opacity:this.state.opacity}]}>
